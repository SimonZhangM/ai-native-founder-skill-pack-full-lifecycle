...
最简单的安装方式在 Windows 和 macOS 上是一样的：

1. 打开 GitHub 仓库页面。
2. 点击 `Code` -> `Download ZIP`。
3. 解压 ZIP 文件。
4. 把解压出来的 pack 内容复制到你的目标项目根目录。

如果你发布的是 release ZIP，并且 ZIP 根目录已经直接包含 pack 文件，
用户可以直接解压到目标项目目录。

如果使用 GitHub 自动生成的 `Download ZIP`，GitHub 通常会多包一层顶级目录，
例如 `ai-native-founder-skill-pack-main/`。这种情况下，先打开
这个目录，再把里面的内容复制到目标项目根目录。
...
全量安装可能覆盖目标项目已有的 `AGENTS.md`、`README.md` 或
`.codex/config.toml`。如果目标项目已经有这些文件，建议先备份或手动合并。

然后从这个项目根目录重新打开一个 Codex session，让 Codex 读取新的项目规则、Skills 和 Agents。

### macOS / Linux 命令行安装，可选

大多数用户不需要这一段。只有在你习惯用 Terminal，并且本地已经下载或解压了
pack 时，才需要命令行方式。

先进入已经解压出来的 pack 目录，再运行：
...
把 `TARGET` 替换成真实的目标项目路径。这个命令会复制整套 pack，包括
runtime 文件、docs、examples、assets、README 和 MANIFEST。

### 只安装 runtime

如果目标项目已经有自己的 README、docs 和 examples，只想让 Codex 读取这套
founder workflow runtime，可以只复制运行时文件：
...
这里的 `/.` 是有意保留的：它表示复制目录里的内容，避免生成
`.agents/skills/skills` 这种嵌套目录。
...
- `docs/promptbase-packaging.md`：PromptBase 等市场平台的打包建议。
